...
class ElderlyInterface:
    """
    老年人友善介面管理器
    整合Rocket.Chat的即時通訊功能
    """
    ...
    def __init__(self):
        ...
        self.interface_settings = {
            ...
            'voice_enabled': os.environ.get('ELDERLY_VOICE_ENABLED', 'true').lower() == 'true',
        ...
        # 語音辨識設定
        self.speech_recognizer = sr.Recognizer() if self.interface_settings['voice_enabled'] else None
        
        # 初始化語音播放
        pygame.mixer.init()
        
        logger.info("老年人友善介面初始化完成")
    ...
    def process_voice_input(self, audio_file) -> str:
        ...
        Returns:
            辨識結果文字
        """
        if not self.interface_settings['voice_enabled']:
            return "語音功能已停用"
        ...
        try:
            ...
            logger.info(f"語音辨識成功: {text}")
            return text
            
        except sr.UnknownValueError:
            logger.error("語音辨識無法理解")
            return "抱歉，我無法理解您的語音。請再試一次，或改用文字輸入。"
        
        except sr.RequestError as e:
            logger.error(f"語音辨識服務錯誤: {e}")
            return "語音辨識服務暫時無法使用，請改用文字輸入。"
        
        except Exception as e:
            logger.error(f"語音輸入處理失敗: {e}")
            return "語音輸入處理失敗，請改用文字輸入。"
    ...
    def _send_emergency_notification(self, contact: Dict, message: str) -> Dict:
        ...
        try:
            # 透過Rocket.Chat發送訊息
            if self.rocket_chat_api_token:
                # 使用API發送
                headers = {
                    'X-Auth-Token': self.rocket_chat_api_token,
                    'X-User-Id': 'system',
                    'Content-Type': 'application/json'
                }
                
                payload = {
                    'channel': f"@{contact['name']}",
                    'text': message,
                    'priority': 'high'
                }
                
                response = requests.post(
                    f"{self.rocket_chat_url}/api/v1/chat.postMessage",
                    headers=headers,
                    json=payload
                )
                
                if response.status_code == 200:
                    return {'status': 'success', 'method': 'rocket_chat'}
                else:
                    logger.warning(f"Rocket.Chat通知失敗: {response.status_code}")
            
            # 如果Rocket.Chat失敗，使用備用方法（這裡只是模擬）
            logger.info(f"透過備用方法通知 {contact['name']}")
...
if __name__ == '__main__':
    ...
    print("介面設定:", json.dumps(settings, indent=2, ensure_ascii=False))
    ...
    print("緊急聯繫結果:", json.dumps(result, indent=2, ensure_ascii=False))
